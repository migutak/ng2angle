...
export class NotesComponent implements OnInit {
  ...
  getbulknotes(custnumber) {
    this.ecolservice.totaluploaded(custnumber).subscribe(data => {
      this.bulknotelength = data[0].TOTAL;
    });
  }

  getflagged(cust) {
    this.ecolservice.getflaggednotes(cust).subscribe(data => {
      this.flaggedlength = data[0].TOTAL;
    });
  }

  //writen comment count
  getNotes(custnumber) {
    this.ecolService.totalcomments(custnumber).subscribe(data => {
