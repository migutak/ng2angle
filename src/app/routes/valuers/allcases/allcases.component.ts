...
export class AllCasesComponent implements OnInit {
    ...
    constructor(private modalService: NgbModal,
        ...
        this.columnDefs = [
            ...
            { field: 'VEHICLEREGNO', filter: 'agTextColumnFilter', filterParams: { newRowsAction: 'keep' }, resizable: true },
            { field: 'VEHICLEMAKE', filter: 'agTextColumnFilter', filterParams: { newRowsAction: 'keep' }, resizable: true },
            { field: 'YARD', filter: 'agTextColumnFilter', filterParams: { newRowsAction: 'keep' }, resizable: true },
            { field: 'REGOWNER', filter: 'agTextColumnFilter', filterParams: { newRowsAction: 'keep' }, resizable: true }
    ...
    onGridReady(params) {
        ...
        const datasource = {
            getRows(params) {
                fetch(environment.api + '/api/tblvaluers/gridviewall', {
                    method: 'post',
    ...
    public ngOnInit(): void {
        ...
        // get sptypes
        this.ecolService.getsptype('VALUERS').subscribe(data => {
            this.sptypes = data;
        });
    }
    ...
    onRowSelected(event) {
        ...
        if (selectedData.length == 1) {
            ...
            this.data.valuationdate = selectedData[0].VALUATIONDATE;
            this.data.forcedsalevalue = selectedData[0].FORCEDSALEVALUE;
            this.data.openmarketvalue = selectedData[0].OPENMARKETVALUE;
    ...
    updateValuer(form) {
        ...
        this.data.openmarketvalue = form.value.openmarketvalue;
        this.data.forcedsalevalue = form.value.forcedsalevalue;
        this.data.valuationdate = moment(form.value.valuationdate).add(1, 'days').format('YYYY-MM-DD');
        
        this.ecolService.patchvaluation(this.data).subscribe(resp => {
