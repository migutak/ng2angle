...
export class NewcaseComponent implements OnInit {
    ...
    ngOnInit() {
        // set default
        this.model.propertytype = 'VEHICLE';
        // get sptypes
        this.ecolService.getsptype('STORAGE YARDS').subscribe(data => {
            this.yards = data;
        });
    }
    ...
    onSubmit(form) {
        ...
        this.ecolService.checkinvaluation(body.accnumber).subscribe(resp => {
            if (resp && resp.length>0) {
                swal('Warning!', 'There exist a running Request for this account ' + body.accnumber, 'warning');
                return;
            } else {
                this.ecolService.newvaluation(body).subscribe(data => {
