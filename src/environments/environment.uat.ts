export const environment = {
  production: true,
  adlogin: false,
  api: 'http://172.16.19.151:8000',
  mongo: 'http://172.16.19.151',
  nodeapi: 'http://172.16.19.151:6100/nodeapi',
  letters_path: '/home/ecollectadmin/demand_letters/',
  letters_api: 'http://172.16.19.151:8004/docx/',
  uploadurl: 'http://172.16.19.151/filesapi',
  demanddownload: 'http://172.16.19.151/demanddownload',
  valor: 'http://172.16.19.151/valor',
  metrics: 'http://172.16.19.151/appmetrics-dash',
  applink: 'http://172.16.19.151',
  xlsuploadapi: 'http://172.16.19.151/xlsupload/',
  xlstemplate: '/home/ecollectadmin/templates/upload_notes.xlsx',
  emailapi: 'http://172.16.19.151/demandemail/email',
  auth: 'http://172.16.19.151/adlogin',
  filesapi: 'http://172.16.19.151/filesapi',
  accplanlink: 'http://ecollectapp.co-opbank.co.ke:3001',
  kibanarpt: 'http://172.16.19.151:5601',
  manuals_path: '/app/manuals/',
  birt: 'http://172.16.19.151:8787/birt',
  kibana: 'http://172.16.19.151:5601/app/kibana#/dashboard?_g=0',
};
